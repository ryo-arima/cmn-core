...
func (rcvr *groupRepository) Upsert(ctx context.Context, uuid, name string) error {
	now := time.Now()
	var row model.PgGroups
	err := rcvr.db.WithContext(ctx).Where("uuid = ? AND deleted_at IS NULL", uuid).First(&row).Error
	if err == nil {
		return rcvr.db.WithContext(ctx).Model(&row).Updates(map[string]interface{}{
			"name":       name,
			"updated_at": &now,
		}).Error
	}
	if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		return rcvr.db.WithContext(ctx).Create(&row).Error
	}
	return err
}

func (rcvr *groupRepository) LookupName(ctx context.Context, uuid string) string {
	var row model.PgGroups
	if err := rcvr.db.WithContext(ctx).Where("uuid = ? AND deleted_at IS NULL", uuid).First(&row).Error; err != nil {
		return uuid
	}
	return row.Name
}

func (rcvr *groupRepository) LookupNames(ctx context.Context, uuids []string) map[string]string {
	result := make(map[string]string, len(uuids))
	if len(uuids) == 0 {
		return result
	}
	var rows []model.PgGroups
	if err := rcvr.db.WithContext(ctx).Where("uuid IN ? AND deleted_at IS NULL", uuids).Find(&rows).Error; err == nil {
...
func (rcvr *groupRepository) SoftDelete(ctx context.Context, uuid string) error {
	now := time.Now()
